...
local function activateSpy()
	...
	for _, obj in ipairs(ReplicatedStorage:GetChildren()) do
		if obj:IsA("RemoteFunction") then
			...
			indexRF[obj] = rfCount
			warn("Tracking RemoteFunction #" .. rfCount .. ": " .. obj.Name)
		elseif obj:IsA("RemoteEvent") then
			reCount += 1
			remoteEvents[obj] = true
			indexRE[obj] = reCount
			warn("Tracking RemoteEvent #" .. reCount .. ": " .. obj.Name)
		end
	end
	...
	mt.__namecall = newcclosure(function(self, ...)
		...
		if remoteFunctions[self] and method == "InvokeServer" then
			local idx = indexRF[self] or "?"
			warn("[SPY] 🔵 RemoteFunction #" .. idx .. " | " .. self.Name)
		...
		-- RemoteEvent Spy
		if remoteEvents[self] and method == "FireServer" then
			local idx = indexRE[self] or "?"
			warn("[SPY] 🔴 RemoteEvent #" .. idx .. " | " .. self.Name)
			for i,v in ipairs(args) do
				if typeof(v) == "table" then
					print(" Arg["..i.."] = {")
					printTable(v, 1)
	...
	setreadonly(mt, true)
	print("✅ Spy ACTIVE (RemoteFunctions + RemoteEvents)")
end
